...
const postId = req.body.postId
    ...
    db.query("INSERT INTO like (userLiking, postId) VALUES (?, ? )", [userLiking, postId], (err, results) => {
        if (err) {
            console.log(err)
        }
        db.query("UPDATE uploads SET likes = likes + 1 WHERE id =?", postId, (err2, result2) => {
            res.send(results)
        })
